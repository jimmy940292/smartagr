...
def parse_log_from_file(senderLogFile):
    ...
    return throughputList[:10], latency, packetlossrate, rssi, snr
...
def draw_avg_bar(senderLogFiles):
    ...
    labels = ["Run 1", "2", "3"]
    # Throughput
    # avgThroughputList = pd.DataFrame(avgThroughput
    for i in range(3):
        plt.plot(x, avgThroughputList[i], color=colors[i % len(colors)], label=str(i), linestyle="--",
             marker=markers[i % len(markers)], linewidth=10, markersize=80, markevery=1)
    ...
    plt.xticks([0, 2, 4, 6, 8, 10], fontsize=my_fontsize)
    ...
    plt.tight_layout()
    plt.savefig(figFolder + distance + "throughput_times_wifi.svg", dpi=300, bbox_inches="tight")
    plt.savefig(figFolder + distance +"throughput_times_wifi.eps", dpi=300, bbox_inches="tight")
    plt.clf()
