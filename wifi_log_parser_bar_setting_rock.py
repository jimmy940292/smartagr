...
def parse_log_from_file(senderLogFile):
    ...
    for i, line in enumerate(open(senderLogFile, 'r')):
        if(re.match(pattern1, line)):
            ...
            throughputList.append(float(re.match(pattern2, line).group(2))*1000.0)
...
def draw_avg_bar(senderLogFiles):
    ...
    # Label
    labels = []
    for i in range(len(avgLatencyList) - 1):
        labels.append(str(i))
    labels.append("Avg")
    ...
    avgThroughputList = avgThroughputList.T
    ...
    plt.savefig(figFolder + prefix+ distance + "throughput_runs_wifi.svg", dpi=300, bbox_inches="tight")
    plt.savefig(figFolder + prefix+distance +
                "throughput_runs_wifi.eps", dpi=300, bbox_inches="tight")
    plt.clf()
    ...
    plt.xticks(x, labels, fontsize=my_fontsize)
    plt.yticks(fontsize=my_fontsize)
    plt.savefig(figFolder +prefix+distance + "latency_runs_wifi.svg", dpi=300, bbox_inches="tight")
    plt.savefig(figFolder + prefix + distance + "latency_runs_wifi.eps",
    ...
    plt.yticks(fontsize=my_fontsize)
    # plt.ylim(0.0, 100.0)
    plt.savefig(figFolder + prefix+distance + "packetlossrate_runs_wifi.svg",
                dpi=300, bbox_inches="tight")
    plt.savefig(figFolder + prefix+distance + "packetlossrate__runs_wifi.eps",
                dpi=300, bbox_inches="tight")
    plt.clf()
    ...
    plt.savefig(figFolder + prefix+distance +
                "rssi_runs_wifi.svg", dpi=300, bbox_inches="tight")
    plt.savefig(figFolder + prefix+distance + "rssi_runs_wifi.eps",
                dpi=300, bbox_inches="tight")
    plt.clf()
...
if __name__ == "__main__":
    ...
    for i in range(args.expNumber + 1):
        s_file = logFolderName + distance+ senderLogFileName + "_" + str(i) + ".log"
